...
use crate::lru_disk_cache::lru_cache::{LruCache, Meter};
...
impl LruDiskCache {
    ...
    pub fn add_file(&mut self, addfile_path: AddFile<'_>, size: u64) -> Result<()> {
        ...
        info!("add_file rel_path: {:?}", rel_path);
    ...
    pub fn remove<K: AsRef<OsStr>>(&mut self, key: K) -> Result<Option<()>> {
        match self.lru.remove(key.as_ref()) {
            Some(_) => {
                let path = self.rel_to_abs_path(key.as_ref());
                match fs::remove_file(&path).map_err(|e| {
